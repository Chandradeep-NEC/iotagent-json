...
function parseData(req, res, next) {
    var data, error, payload, datalist, dataArray;
    dataArray = [];
    if (req.body) {
        datalist = JSON.stringify(req.body);
        try {
    ...
    if (error) {
        ...
        config.getLogger().debug(context, 'Parsed data: [%j]', dataArray);
        next();
    }   
...
function addTimestamp(req, res, next) {
    var arr = req.jsonPayload;
    var timeStampData, dataWithTimeStampArr;
    dataWithTimeStampArr = [];
    for (var i in arr) {
...
function handleIncomingMeasure(req, res, next) {
    ...
    function processHTTPWithDevice(device) {
        var payloadDataArr, attributeArr, attributeValues;
        attributeArr = [];
        payloadDataArr = req.jsonPayload;
        if (req.jsonPayload) {
            for (var i in payloadDataArr) {
            ...
            for (var j in attributeArr) {
                ...
                for (var k in attributeValues) {
                    executions.push(iotAgentLib.setCommandResult.bind(
                        null,
                        device.name,
